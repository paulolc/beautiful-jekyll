...
var Lang = {
	...
	langCode:'pt', //(  new URLSearchParams(window.location.search).get('lang') || navigator.language || navigator.userLanguage).substr(0,2),
	...
	trans: {
		...
		de: {
			...
			getButton:				'Schalter %m.inputs gedrÃ¼ckt?',
		...
		es: {
			...
			setLampVal:            'fijar lÃ¡mpara %m.outputs a %m.outputValues',
		...
		ca: {
			...
			setLampVal:            'fixa lÃ mpada %m.outputs a %m.outputValues',
			...
			getOutputValue:        'obtÃ©n valor de sortida %m.outputs',
		...
		fr: {
			...
			evtLightBarrier:		'barriÃ¨re optique %m.inputs %m.lightBarrierStates', 
			
			setLampVal:				'mettre la lampe %m.outputs Ã  %m.outputValues',
			setMotorValDir:			'rÃ©gler le moteur %m.outputs Ã  %m.outputValues %m.outputDirections',
			setMotorDir:			'paramÃ©trer le moteur %m.outputs pour %m.outputDirections',
			setOutputVal:			'mettre la sortie %m.outputs Ã  %n',
			reset:					'remise Ã  zero',
			
			getButton:				'bouton %m.inputs appuyÃ©?',
			getLightBarrier:		'barriÃ¨re optique %m.inputs coupÃ©e?',
